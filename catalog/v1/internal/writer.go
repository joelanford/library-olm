...
func (w *ContentWriter) AddSuccessor(graphID int64, fromBundleID, toBundleID string) error {
	...
	if err != nil {
		return fmt.Errorf("adding successor %q -> %q in graph %d: %w", fromBundleID, toBundleID, graphID, err)
	}
	return nil
}
...
func DeleteCatalogContent(tx *sql.Tx, catalogName string) error {
	// Delete in FK-safe order: successors, graph_bundles, graphs, bundles
	stmts := []string{
