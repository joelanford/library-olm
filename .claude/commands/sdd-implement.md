...
Read the work item's README.md (and any other files in the spec directory). Verify it has:
- Requirements
- Implementation Plan
- Verification steps
- Acceptance Criteria

If the spec is incomplete (still an `idea` or missing sections), stop and tell the user to run `/sdd-plan-next-phase` to refine it first.
...
Follow the Implementation Plan task groups in order. For each task group:
...
1. Walk through each Verification step in the spec and confirm it passes.
2. Walk through each Acceptance Criterion and confirm it is met.
3. Run `go test ./...` and `go vet ./...` one final time.
4. If any verification step or criterion fails, amend the fix into the commit where the issue was introduced rather than creating a new fixup commit.
